...
export const StudentModal = ({ onClose, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      const res = await api.post('/users/students', formData);
      if (res.data.success) {
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-[#E8E2D5] animate-in fade-in zoom-in duration-200">
        <div className="px-6 py-4 border-b border-[#E8E2D5] flex items-center justify-between bg-[#F5F0E8]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold font-serif text-stone-900 text-base">Register Student</h3>
              <p className="text-xs text-stone-500">Create new student user account & academic profile</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-3.5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full text-xs px-3 py-2.5 rounded-xl border border-[#E8E2D5] outline-none focus:ring-2 focus:ring-amber-700 bg-[#FDFCFB] text-stone-900 font-medium"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-stone-700 uppercase tracking-wider mb-1.5">Default Password *</label>
              <input
                type="text"
                required
