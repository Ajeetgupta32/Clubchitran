...
export const StudentLayout = () => {
  ...
  const navItems = [
    ...
    { to: '/student/activities', label: 'Club Activities', icon: Compass },
    { to: '/student/submissions', label: 'My Proofs & Status', icon: Camera },
    ...
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col md:flex-row text-stone-900">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#18181B] border-r border-stone-800 shrink-0 select-none shadow-xl text-stone-200">
        ...
        <div className="p-5 border-b border-stone-800/80 flex items-center justify-between">
          <ClubLogo size="md" subtitle="Student Portal" dark={true} />
        </div>

        {/* Student Points Pill */}
        <div className="mx-3 my-3 p-3 rounded-2xl bg-stone-900/90 border border-stone-700/80 shadow-inner">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-stone-300 flex items-center gap-1.5">
              <Trophy className="w-3.5 h-3.5 text-amber-400" />
              Club Points
            </span>
            <span className="text-xs font-black text-amber-300 bg-amber-500/15 px-2 py-0.5 rounded-md border border-amber-500/25">
              {user?.student?.points || 0} pts
            </span>
          </div>
          {user?.student?.awardStatus && (
            <p className="text-[10px] font-bold text-emerald-400 mt-1.5 line-clamp-1 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
              ★ {user.student.awardStatus}
            </p>
          )}
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              <div className="truncate">
                ...
                <p className="text-[10px] text-stone-400 truncate">{user?.student?.studentId}</p>
              </div>
            </div>
            <button
      ...
      <header className="md:hidden bg-[#18181B] border-b border-stone-800 px-4 py-3 flex items-center justify-between sticky top-0 z-40 text-white">
        <ClubLogo size="sm" subtitle="Student" dark={true} />
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#18181B] border-b border-stone-800 p-4 space-y-1 animate-in fade-in slide-in-from-top-2 duration-150 text-white">
          <div className="p-3 mb-2 rounded-xl bg-stone-900 border border-stone-700/80 flex items-center justify-between">
            <span className="text-xs text-stone-300 font-semibold">Total Points</span>
            <span className="text-xs font-black text-amber-300">{user?.student?.points || 0} pts</span>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-x-hidden bg-[#FAF8F5]">
        ...
        <header className="hidden md:flex h-16 bg-[#FAF8F5]/90 backdrop-blur-md border-b border-[#E8E2D5] px-8 items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-2 text-xs text-stone-500">
            <span className="font-medium text-stone-500">Student</span>
            <span>/</span>
            <span className="font-bold text-stone-900">{user?.student?.branch || 'Campus Hub'}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl border border-[#E8E2D5] text-xs text-stone-800 shadow-xs font-semibold">
              ...
              <span>{user?.student?.points || 0} Club Points</span>
            </div>
          </div>
        </header>
